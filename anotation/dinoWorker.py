...
def saveYoloSegmentation(masks, image_path, class_id): 
    ...
    with open(txt_path, "w") as f: 
        for mask in masks: 
            mask_np = mask[0].cpu.numpy().astype(np.unit8)
            contours, _ = cv2.findCountours(
                mask_np, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            for countour in contours: 
                if len(countour) < 3: 
                    continue
                polygon = countour.reshape(-1, 2) / np.array([w, h])
                poly_str = " ".join(
                    f"{c[0]:.6f} {c[1]:.6f}" for c in polygon
                )
...
def annotateAndSaveVisual(image_source, masks, image_path): 
    ...
    cv2.imwrite(str(visual_path), cv2.cvtColor(result, cv2.COLOR_BAYER_BG2BGR))
...
def annotateImage(image_path, prompt, class_id, task_id, frame): 
    ...
    transformed = sam_predictor.transform.apply_boces_torch(
...
#A prompt, an the images. We will consume one image at a time. 
#We may implment strategies for mutliple workers consuming at the same time, fan_out

#PathImages = AI-Cup-\blender\staging


def onAnnotationJob(ch, mehtod, properties, body): 
    job = json.loads(body)
    taskId = job["taskId"]
    frame = job["frame"]
    imagePath = job["path"]
    ...
    if not promptRaw: 
        ...
        ch.basic_nack(delivery_tag=mehtod.delivery_tag, requeue=False)
        return
